...
{
	static float elapsedTime = 0.f, fogBlendingTime = 10.f;
	...
	mMainPassCB.gFogStart += 10.f * gt.DeltaTime();
...
void BlendApp::LoadTextures()
{
	...
	boxTex->Name = "boxTex";
	boxTex->Filename = L"../../Textures/bricks2.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
...
void BlendApp::BuildMaterials()
{
	...
	auto alphabox = std::make_unique<Material>();
	alphabox->Name = "alphabox";
	alphabox->MatCBIndex = 3;
	alphabox->DiffuseSrvHeapIndex = 3;
	alphabox->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 0.5f);
	alphabox->FresnelR0 = XMFLOAT3(0.1f, 0.1f, 0.1f);
	alphabox->Roughness = 0.25f;

	auto defaultbox = std::make_unique<Material>();
	defaultbox->Name = "defaultbox";
	defaultbox->MatCBIndex = 4;
	defaultbox->DiffuseSrvHeapIndex = 3;
	defaultbox->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	defaultbox->FresnelR0 = XMFLOAT3(0.1f, 0.1f, 0.1f);
	defaultbox->Roughness = 0.25f;

	mMaterials["grass"] = std::move(grass);
	mMaterials["water"] = std::move(water);
	mMaterials["wirefence"] = std::move(wirefence);
	mMaterials[alphabox->Name] = std::move(alphabox);
	mMaterials[defaultbox->Name] = std::move(defaultbox);
}

void BlendApp::BuildRenderItems()
{
	auto boxRitem3 = std::make_unique<RenderItem>();
	XMStoreFloat4x4(&boxRitem3->World, XMMatrixTranslation(15.0f, 1.0f, -25.0f));
	boxRitem3->ObjCBIndex = 5;
	boxRitem3->Mat = mMaterials["alphabox"].get();
	...
	XMStoreFloat4x4(&boxRitem1->World, XMMatrixTranslation(25.0f, 2.0f, -20.0f));
	...
	XMStoreFloat4x4(&boxRitem2->World, XMMatrixTranslation(-15.0f, 2.0f, 15.0f));
	boxRitem2->ObjCBIndex = 4;
	boxRitem2->Mat = mMaterials["defaultbox"].get();
